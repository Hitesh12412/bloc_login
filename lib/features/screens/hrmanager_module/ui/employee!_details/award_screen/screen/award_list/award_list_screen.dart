import 'package:bloc_login/features/screens/hrmanager_module/ui/employee!_details/award_screen/bloc/award_list_bloc/award_list_bloc.dart';
...
import 'package:bloc_login/features/screens/hrmanager_module/ui/employee!_details/award_screen/screen/award_list/delete_award_bottom_sheet.dart';
...
class AwardListScreenWidget extends StatefulWidget {
  final String employeeId;

  const AwardListScreenWidget({
    required this.employeeId,
    super.key,
  });

  @override
  State<AwardListScreenWidget> createState() => _AwardListScreenWidgetState();
}

class _AwardListScreenWidgetState extends State<AwardListScreenWidget> {
  ...
  @override
  void initState() {
    BlocProvider.of<AwardBloc>(context).add(
      FetchAwardEvent(employeeId: widget.employeeId),
    );
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Stack(
        children: [
          BlocBuilder<AwardBloc, AwardStates>(
            builder: (context, state) {
              if (state is LoadingAwardState) {
                return const Center(child: CircularProgressIndicator());
              } else if (state is LoadedAwardState) {
                final Awards = state.model.data;
                if (Awards.isNotEmpty) {
                  return ListView.builder(
                    itemCount: Awards.length,
                    itemBuilder: (context, index) {
                      final Award = Awards[index];
                      final docUrl = Award.documentUpload;
                                      ...
                                      Column(
                                        ...
                                        children: [
                                          ...
                                          Container(
                                              ...
                                              child: const Text('Award',style: TextStyle(color: Colors.blue),))
                                        ],
                                      ),
                                  const Spacer(),
                              ...
                              child: Row(
                                children: [
                                  Container(
                                      ...
                                      child: const Icon(Icons.calendar_today_rounded,color: Colors.blue,size: 25,),
                                  ),
                                  const SizedBox(width: 10,),
                                  Column(
                                    crossAxisAlignment: CrossAxisAlignment.start,
                                    children: [
                                      const Text('Year of Award Received',style: TextStyle(color: Colors.grey),),
                                      Text(Award.yearOfReceivingAward,style: const TextStyle(fontSize: 18,fontWeight: FontWeight.bold),)
                                    ],
                                  )
                                ],
                              ),
                                    ...
                                    children: [
                                      Container(
                                        ...
                                        child: Row(
                                          children: [
                                            Text(
                                              Award.documentUpload.split('/').last,
              ...
              } else if (state is FailureAwardState) {
                return Center(child: Text('Error: ${state.error}'));
              } else if (state is InternalServerErrorAwardState) {
                return Center(child: Text(state.error));
              } else if (state is ServerErrorAwardState) {
                return Center(child: Text(state.error));
              }
              return Container();
            },
