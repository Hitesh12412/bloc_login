import 'package:bloc_login/features/screens/hrmanager_module/ui/employee!_details/insurance_screen/bloc/insurance_list_bloc/insurance_list_bloc.dart';
...
import 'package:bloc_login/features/screens/hrmanager_module/ui/employee!_details/insurance_screen/screen/insurance_list_screen/delete_insurance_bottom_sheet.dart';
...
class _InsuranceListScreenState extends State<InsuranceListScreen> {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      child: InsuranceListScreenWidget(
...
class InsuranceListScreenWidget extends StatefulWidget {
  final String employeeId;

  const InsuranceListScreenWidget({
    required this.employeeId,
    super.key,
  });

  @override
  State<InsuranceListScreenWidget> createState() => _InsuranceListScreenWidgetState();
}

class _InsuranceListScreenWidgetState extends State<InsuranceListScreenWidget> {
  ...
  void initState() {
    BlocProvider.of<InsuranceBloc>(context).add(
      FetchInsuranceEvent(employeeId: widget.employeeId),
    );
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Stack(
        children: [
          BlocBuilder<InsuranceBloc, InsuranceStates>(
            builder: (context, state) {
              if (state is LoadingInsuranceState) {
                return const Center(child: CircularProgressIndicator());
              } else if (state is LoadedInsuranceState) {
                final Insurances = state.model.data;
                if (Insurances.isNotEmpty) {
                  return ListView.builder(
                    itemCount: Insurances.length,
                    itemBuilder: (context, index) {
                      final Insurance = Insurances[index];
                      final docUrl = Insurance.documentUpload;
                                    ...
                                    itemBuilder: (_) => [
                                      ...
                                      PopupMenuItem<String>(
                                        onTap: () => showModalBottomSheet(
                                          ...
                                          builder: (context) {
                                            return DeleteInsuranceBottomSheet(
                                              Insurance: Insurance,
                                              onDeleteConfirmed: () {
                                                ...
                                                ScaffoldMessenger.of(context)
                                                  ...
                                                  SnackBar(
                                                    content: Text(
                                                        'Deleted Insurance: ${Insurance.companyName}'),
                                                  ),
                                                );
                                              },
                                            );
                                    ...
                                    children: [
                                      Container(
                                        ...
                                        child: Row(
                                          children: [
                                            Text(
                                              Insurance.documentUpload.split('/').last,
              ...
              } else if (state is FailureInsuranceState) {
                return Center(child: Text('Error: ${state.error}'));
              } else if (state is InternalServerErrorInsuranceState) {
                return Center(child: Text(state.error));
              } else if (state is ServerErrorInsuranceState) {
                return Center(child: Text(state.error));
              }
              return Container();
            },
